...
import { logJiraWebhookEvent } from "@/lib/connectors/corporate/jira";
...
function errorResponse(error: unknown) {
  ...
  console.error("[webhooks:jira:events]", error);
...
export async function POST(request: NextRequest) {
  try {
    const limited = rateLimit(request, {
      maxRequests: 120,
      windowMs: 60_000,
      keyPrefix: "webhooks:jira:events",
    });
    ...
    if (!verifyCorporateWebhookSecret(bearerSecret(request), connector)) {
      return NextResponse.json({ error: "Invalid Jira webhook secret" }, { status: 401 });
    }

    const deliveryId =
      request.headers.get("x-atlassian-webhook-identifier") ??
      request.headers.get("x-request-id") ??
      asString(body.webhookIdentifier);
    const result = await logJiraWebhookEvent({
      connector,
      payload,
      eventName: asString(body.webhookEvent) ?? asString(body.eventType),
      deliveryId,
    });
